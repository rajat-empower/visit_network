...
/**
 * GET /api/places-to-stay/[city]/[name]
 * Returns details for a specific place to stay by city and name
 */
export async function GET(
  ...
  try {
    ...
    if (!city || !name) {
      return NextResponse.json({ error: 'City and place name are required' }, { status: 400 });
    }

    const formattedName = formatNameForQuery(name);
    const formattedCity = formatNameForQuery(city);
    
    console.log("Fetching place with:", { 
      originalName: name, 
      originalCity: city,
      formattedName, 
      formattedCity 
    ...
    if (cityError) {
      ...
      // Now try to find the place with this city
      const { data: placeData, error: placeError } = await supabase
        ...
        .select(`
          *,
          ...
          type:place_types(*)
        `)
        .ilike('name', `%${formattedName.split(' ')[0]}%`) // Try matching just the first word of the place name
        .eq('city_id', cityMatch.id)
        .limit(1);
        
      if (placeError || !placeData || placeData.length === 0) {
        console.error("Place not found with fuzzy search:", { formattedName, cityId: cityMatch.id, error: placeError });
      ...
      console.log("Found place with fuzzy search:", placeData[0]);
    ...
    // Try to find the place with exact name match
    const { data: placeData, error: placeError } = await supabase
      .from('places_to_stay')
      .select(`
        *,
    ...
    if (placeError) {
      console.error("Place not found with exact match, trying fuzzy search:", { formattedName, cityId: cityData.id, error: placeError });
      
      // Try a more flexible search for the place
      ...
      if (placesError || !placesData || placesData.length === 0) {
        console.error("Place not found with fuzzy search:", { formattedName, cityId: cityData.id, error: placesError });
        return NextResponse.json({ error: 'Place not found' }, { status: 404 });
      }
      ...
      console.log("Found place with fuzzy search:", placesData[0]);
      return NextResponse.json({ 
        data: placesData[0],
        relatedTours 
      });
    ...
    console.log("Found place with exact match:", placeData);
    return NextResponse.json({ 
      data: placeData,
      relatedTours 
    });
