...
// Get an existing tour type to use
const getExistingTourType = async () => {
  // Fetch all tour types
  const { data: tourTypes, error } = await supabase
    .from('tour_types')
    .select('id, name');
  ...
  if (!tourTypes || tourTypes.length === 0) {
    console.error('No tour types found in the database');
    throw new Error('No tour types found in the database');
  }
  
  console.log('Available tour types:', tourTypes);
  
  // Use the first tour type
  return tourTypes[0].id;
};

// Make sure the additional tour exists
const ensureAdditionalTour = async (generalTourTypeId) => {
  // Additional tour from RecommendedActivities
  const additionalTour = {
    ...
    tour_type_id: generalTourTypeId,
    is_featured: true
  };
...
const generateNewTour = (existingTour) => {
  // Generate a new ID
  const newId = uuidv4();
...
export async function replicateTours() {
  try {
    ...
    const generalTourTypeId = await ensureGeneralTourType();
    console.log('Ensured general tour type exists with ID:', generalTourTypeId);
    
    // Ensure the additional tour exists
    await ensureAdditionalTour(generalTourTypeId);
